...
bool DiscreteElasticRod::getConstraintGradient(const Optimization::VectorXf &x_lambda, Optimization::VectorXf &gradient) const {
    ...
    X_constraints = x_lambda.head(3 * (m_n + 2)) - m_vertex_positions.reshaped(3 * (m_n + 2), 1);
...
bool DiscreteElasticRod::getConstraintHessian(const Optimization::VectorXf &x_lambda, Optimization::TripletListF &hessian) const {
    ...
    for (int i = 0; i <= m_n; i++) {
        for (int j = 0; j < 3; j++) {
            int dl_index = 3 * (m_n + 2) + i;
            int dx_index = 3 * i + j;

            float x_i_minus_x_i_plus_one = 2 * (x(dx_index) - x(dx_index + 3));
            float x_i_plus_one_minus_x_i = 2 * (x(dx_index + 3) - x(dx_index));
            ...
            // print_debug(x_i_minus_x_i_plus_one);
            // print_debug(x_i_plus_one_minus_x_i);
        }
